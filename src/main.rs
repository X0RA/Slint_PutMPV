...
use metadata::tmdb::{MovieDetails, TVSeasonDetails, TVSeriesDetails, TvEpisodeAirRef};
...
const VIEW_MEDIA: i32 = 4;
const VIEW_TV_SHOW: i32 = 8;
...
fn parse_season_cache_key(key: &str) -> Option<i32> {
    let rest = key.strip_prefix("season_")?;
    rest.split('_').next()?.parse().ok()
}

fn load_tv_season_details_from_sub(
    sub: &std::collections::HashMap<String, storage::tmdb_store::CacheEntry>,
    season_number: i32,
...
fn build_episode_credits(
    ep: &metadata::tmdb::Episode,
    missing_profiles: &mut Vec<String>,
) -> Vec<TvEpisodeCredit> {
...
fn refresh_tv_show_ui(
    ...
    let tmdb_cache = match tmdb_store.get_cache_snapshot() {
        Ok(c) => c,
        Err(_) => storage::tmdb_store::TMDBCache::default(),
    };
    ...
    let stats_line = if lib_seasons > 0 || lib_eps > 0 {
        ...
        format!(
            ...
            if details.number_of_seasons == 1 { "" } else { "s" },
            details.number_of_episodes,
            if details.number_of_episodes == 1 { "" } else { "s" },
        )
    } else {
        String::new()
    };
    ...
    if let Some(ref sd) = season_details {
        ...
        let matched_here = sel_sn
            .map(|sn| matched_episodes_in_season(sub, sn, &episode_to_file))
            .unwrap_or(0);
        let meta = format!("{} matched · {} episodes · {}", matched_here, sd.episodes.len(), air);
...
fn main() -> Result<()> {
    ...
    let tv_show_seasons_model = Rc::new(VecModel::from(Vec::<TvSeasonTab>::new()));
    let tv_show_episodes_model = Rc::new(VecModel::from(Vec::<TvEpisodeRow>::new()));
    let tv_show_hero_badges_model = Rc::new(VecModel::from(Vec::<TvHeroBadge>::new()));
    let tv_show_hero_creators_model = Rc::new(VecModel::from(Vec::<TvCreatorChip>::new()));
    let tv_show_detail_lines_model = Rc::new(VecModel::from(Vec::<slint::SharedString>::new()));
    let tv_show_networks_model = Rc::new(VecModel::from(Vec::<slint::SharedString>::new()));
    ...
    app.set_tv_show_seasons(ModelRc::from(tv_show_seasons_model.clone()));
    app.set_tv_show_episodes(ModelRc::from(tv_show_episodes_model.clone()));
    app.set_tv_show_hero_badges(ModelRc::from(tv_show_hero_badges_model.clone()));
    app.set_tv_show_hero_creators(ModelRc::from(tv_show_hero_creators_model.clone()));
    app.set_tv_show_detail_lines(ModelRc::from(tv_show_detail_lines_model.clone()));
    app.set_tv_show_networks(ModelRc::from(tv_show_networks_model.clone()));
    ...
    app.on_media_item_clicked({
        ...
        let matched_store = matched_store.clone();
        let tmdb_store = tmdb_store.clone();
        let tv_show_seasons_model = tv_show_seasons_model.clone();
        let tv_show_episodes_model = tv_show_episodes_model.clone();
        let tv_show_hero_badges_model = tv_show_hero_badges_model.clone();
        let tv_show_hero_creators_model = tv_show_hero_creators_model.clone();
        let tv_show_detail_lines_model = tv_show_detail_lines_model.clone();
        let tv_show_networks_model = tv_show_networks_model.clone();
        let rt = rt.clone();
        move |file_id| {
            let Some(app) = weak.upgrade() else {
                return;
    ...
    app.on_tv_show_episode_subtitles({
        ...
        move |file_id| {
            ...
            if let Ok(id) = s.parse::<u64>() {
                if let Some(app) = weak.upgrade() {
                    // Opens player like Play; pick subtitles from player UI.
                    app.invoke_files_menu_action("play".into(), truncate_id(id));
                }
            }
        }
